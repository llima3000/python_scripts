...
def get_vs_list(api, api_version, se_group):
    vs_list = []
    #/api/virtualservice?page_size=1000&se_group_ref__name__contains=name
    ...
    return vs_list
def vs_scalein(api, api_version, log, vs_uuid):
    ...
    #log.info("VS %s scaled in " % (vs_uuid))
        
def main():
    requests.packages.urllib3.disable_warnings()
    log = get_root_logger(__name__, logfile_path='/opt/avi/log/vs_scaleout.log',
                          level=logging.INFO,
                          propagate=True)
    #Getting Required Args
    parser = argparse.ArgumentParser(description="AVISDK based Script to scale out VSes with Single SEs")
    parser.add_argument("-c", "--controller", required=False, default="localhost", help="Controller IP address")
    ...
    parser.add_argument("-s", "--se_group", required=False, default="Default-Group", help="Service Engine Group name")
    ...
    for vs_uuid in vs_list:
        vs_scalein(api, api_version, log, vs_uuid)
